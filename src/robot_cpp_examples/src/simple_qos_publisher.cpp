...
class SimepleQoSPublisher: public rclcpp::Node
{
    public:
    SimepleQoSPublisher():Node("simple_qos_publisher"), qos_profile_pub_(10), counter_(0)
    {
        ...
        pub_ = create_publisher<std_msgs::msg::String>("chartter", qos_profile_pub_);
        timer_ = create_wall_timer(1s, std::bind(&SimepleQoSPublisher::timerCallback, this));
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<SimepleQoSPublisher>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return (0);
}