#include "robot_planner/dijkstra_planner.hpp"
...
namespace robot_planner
{
    DijkstraPlanner::DijkstraPlanner():Node("dijkstra_node")
    {
        ...
        map_sub_ = create_subscription<nav_msgs::msg::OccupancyGrid>("/map",
            map_qos, std::bind(&DijkstraPlanner::mapCallback, this, std::placeholders::_1 ));
    ...
    void goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr pose)
    {
        ...
        } catch (const tf2::TransformExecption & ex) {
        ...
        if (!path.poses.empty()){
            ...
            RCLCPP_ERROR(get_logger(), "No path found to the goal !!");
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<robot_planner::DijkstraPlanner>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return (0);
}